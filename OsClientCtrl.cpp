    ...
    switch (head.msgId)
    {
        case messageSimplePingResult :
        {
			...
			if (head.payloadSize + sizeof(OculusMessageHeader) == imageOffset + imageSize) {
				m_pImage = (unsigned char*) realloc(m_pImage,  imageSize);
			  ...
			  if (m_rff.head.payloadSize + sizeof(OculusMessageHeader) == m_rff.ping_params.imageOffset + m_rff.ping_params.imageSize)
				{
				  ...
				  m_pImage = (unsigned char*) realloc(m_pImage,  m_rff.ping_params.imageSize);
...
void OsReadThread::Startup()
{
  ...
  {
    SetActive(true);
    //start(); // NOT required for std::thread
  }
}
...
void OsReadThread::run()
{
  ...
  serverTCP.sin_addr.s_addr = inet_addr(m_hostname.c_str());
...
void OsClientCtrl::WriteToDataSocket(char* pData, uint16_t length)
{
